...
const Page = () => {
  ...
  const handleUpdateBanner = async () => {
    ...
    try {
      ...
      formData.append("pageName", "French");
      if (inputValues.bannerImage) {
        formData.append("bannerImage", inputValues.bannerImage as any);
      }
      const response = await axios.patch(`http://3.110.121.13/api/banner/update/French`, formData);
  ...
  const updatedData = data.filter((item: any) => item.pageName === "French");
