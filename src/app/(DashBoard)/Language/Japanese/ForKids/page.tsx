...
const Page = () => {
  ...
  const faqData = data.filter(
    (item: any) => item.language === "Japanese" && item.context === "Kids" && item.category === "General" 
  );
  const infoSectionData = data.filter(
    (item: any) =>
      item.category === "Everything you need to know" && item.context === "Kids" && item.language === "Japanese"
  );
  ...
  return (
    <div className="w-full h-full flex flex-col gap-5 p-4">
      ...
      {/* FAQ&apos;s */}
      <FormSection data={faqData} title="FAQ's" onSubmit={handleFAQSubmit} context={"Kids"} language={"Japanese"}/>
      <div className="py-10">
      <FormSection
        data={infoSectionData}
        category ={"Everything you need to know"}
        context={"Kids"} language={"Japanese"}
