...
const Page = () => {
  ...
  const faqData = data.filter(
    (item: any) => item.language === "Mandarine" && item.context === "Home" && item.category === "General" 
  );
  const infoSectionData = data.filter(
    (item: any) =>
      item.category === "Everything you need to know" && item.context === "Home" && item.language === "Mandarine"
  );
  ...
  return (
    <div className="w-full h-full flex flex-col gap-5 p-4">
      ...
      {/* FAQ's */}
      <FormSection title="FAQ's" data={faqData} onSubmit={handleFAQSubmit} context={"Home"} language={"Mandarine"} />
      <div className="py-10">
      <FormSection
        data={infoSectionData}
        category ={"Everything you need to know"}
        context={"Home"} language={"Mandarine"}
